    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.clients.update", [$client->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('fname') ? 'is-invalid' : '' }}" type="text" name="fname" id="fname" value="{{ old('fname', $client->fname) }}" required>
                @if($errors->has('fname'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('lname') ? 'is-invalid' : '' }}" type="text" name="lname" id="lname" value="{{ old('lname', $client->lname) }}" required>
                @if($errors->has('lname'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('role') ? 'is-invalid' : '' }}" name="role" id="role" required>
                    ...
                    @foreach(App\Models\Client::ROLE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('role', $client->role) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('role'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('password') ? 'is-invalid' : '' }}" type="text" name="password" id="password" value="{{ old('password', $client->password) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('group') ? 'is-invalid' : '' }}" name="group" id="group">
                    ...
                    @foreach(App\Models\Client::GROUP_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('group', $client->group) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('group'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('crn') ? 'is-invalid' : '' }}" type="text" name="crn" id="crn" value="{{ old('crn', $client->crn) }}">
                @if($errors->has('crn'))
