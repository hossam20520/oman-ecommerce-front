...
@can('filter_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            ...
            @include('csvImport.modal', ['model' => 'Filter', 'route' => 'admin.filters.parseCsvImport'])
        </div>
    </div>
@endcan
<div class="card">
                ...
                <tbody>
                    @foreach($filters as $key => $filter)
                        <tr data-entry-id="{{ $filter->id }}">
                            ...
                            <td>
                                @can('filter_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.filters.show', $filter->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('filter_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.filters.edit', $filter->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('filter_delete')
                                    <form action="{{ route('admin.filters.destroy', $filter->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-Filter:not(.ajaxTable)').DataTable({ buttons: dtButtons })
