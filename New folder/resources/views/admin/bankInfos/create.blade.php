...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.bankInfo.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.bank-infos.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="bankname">{{ trans('cruds.bankInfo.fields.bankname') }}</label>
                <input class="form-control {{ $errors->has('bankname') ? 'is-invalid' : '' }}" type="text" name="bankname" id="bankname" value="{{ old('bankname', '') }}">
